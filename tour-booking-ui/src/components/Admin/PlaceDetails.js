...
import { getPlaceById, editPlaces, deletePlaces } from "../../api/PlaceApi";
...
export default function PlaceDetails() {
  ...
  const onSubmit = async (data) => {
    ...
    const placeRequest = await editPlaces(id, formData);
    if (placeRequest != null) {
      postSuccessToast("Sửa địa điểm thành công!");
      reset(placeRequest);
      setImageList(placeRequest.image);
      setTimeout(() => {
        setPlaceInfo(placeRequest);
        setEditMode(false);
      }, 10);
    }
  };
  ...
  useEffect(() => {
    const PlaceInfo = async () => {
      const tmpPlaceInfo = await getPlaceById(id);
      if (tmpPlaceInfo === null) {
        navigate("/error404");
      }
      setPlaceInfo(tmpPlaceInfo);
      setImageList(tmpPlaceInfo.image);
      reset(tmpPlaceInfo);
    };
  ...
  const handleDelete = async () => {
    await deletePlaces(id);
    postSuccessToast("Xoá địa điểm thành công!");
              ...
              <nav aria-label="breadcrumb">
                <ol className="breadcrumb breadcrumb-no-gutter">
                  <li className="breadcrumb-item">
                    <Link className="breadcrumb-link" to={"/admin/places"}>
                      Địa điểm
                    </Link>
                  </li>
                  <li className="breadcrumb-item active">Thông tin địa điểm</li>
                </ol>
              </nav>
              <h1 className="page-header-title">Địa điểm: {placeInfo.name}</h1>
            </div>
                  ...
                  <div className="form-group">
                    ...
                    <div className="input-group input-group-merge">
                      <input
                        ...
                        id="rateLabel"
                        placeholder="Nhập hạng địa điểm"
                        {...register("rate")}
                        disabled={!editMode}
                      />
                      <div className="error-form-msg">
